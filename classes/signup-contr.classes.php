...
class SignupContr extends Signup {
    ...
    public function signupUser() {
        if($this->emptyInput() == false) {
            header("location: ../error.php?status=Input is empty");
            exit();
        }
        if($this->invalidUid() == false) {
            header("location: ../error.php?status=Username is invalid");
            exit();
        }
        ...
        if($this->pwdMatch() == false) {
            header("location: ../error.php?status=Passwords do not match");
            exit();
        }
        if($this->duplicateUser() == false) {
            header("location: ../error.php?status=Username or email taken");
            exit();
        }
